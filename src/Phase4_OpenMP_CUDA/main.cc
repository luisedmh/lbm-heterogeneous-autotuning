...
const int NY_GPU = static_cast<int>(NY * ALPHA); 
const int NY_CPU = NY - NY_GPU;                  
...
__global__ void streaming_kernel_gpu(const double* f, double* f_next, const bool* obstacle, double u_inflow_param, int ny_sub) {
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    ...
    if (x == 0) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            f_next[i * NX * ny_sub + idx] = d_w[i] * 1.0 * (1.0 + 3.0 * cu + 4.5 * cu * cu - 1.5 * u2);
        }
        return;
    }
    ...
    if (x == NX - 1) {
        ...
        for (int i = 0; i < 9; ++i) {
            ...
            if (prev_y < 0 || prev_y >= ny_sub || obstacle[prev_y * NX + prev_x]) {
                f_next[i * NX * ny_sub + idx] = f[d_noslip[i] * NX * ny_sub + v_idx];
            } else {
                f_next[i * NX * ny_sub + idx] = f[i * NX * ny_sub + (prev_y * NX + prev_x)];
            }
        }
        return;
    }
    ...
    for (int i = 0; i < 9; ++i) {
        ...
        if (prev_y < 0 || prev_y >= ny_sub || obstacle[prev_idx]) {
            f_next[i * NX * ny_sub + idx] = f[d_noslip[i] * NX * ny_sub + idx];
        } else {
            f_next[i * NX * ny_sub + idx] = f[i * NX * ny_sub + prev_idx];
        }
    }
}
...
int main() {
    std::cout << "=========================================================" << std::endl;
    std::cout << ">>> EJECUTANDO FASE 2: CO-EJECUCIÓN HETEROGÉNEA (CPU + GPU)" << std::endl;
    std::cout << "Reparto -> GPU: " << ALPHA*100 << "% | CPU (Ryzen 5): " << (1-ALPHA)*100 << "%" << std::endl;
    std::cout << "=========================================================" << std::endl;
    ...
    for (int step = 0; step < NUM_STEPS; ++step) {
        collision_kernel_gpu<<<gridSize, blockSize, 0, stream_gpu>>>(d_f, d_obstacle, tau, NY_GPU);
        streaming_kernel_gpu<<<gridSize, blockSize, 0, stream_gpu>>>(d_f_next, d_f, d_obstacle, u_inflow, NY_GPU);

        cpu_lbm_step(h_f, h_f_next, h_obs);

        for (int i = 0; i < 9; ++i) {
            cudaMemcpyAsync(h_f_next.data() + i * NX * NY + NY_GPU * NX, 
                            d_f_next + i * NX * NY_GPU + (NY_GPU - 1) * NX, 
                            NX * sizeof(double), cudaMemcpyDeviceToHost, stream_gpu);
        }
